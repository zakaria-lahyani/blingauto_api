...
class AuthService:
    ...
    async def register_user(
        ...
        async with get_db_session() as session:
            ...
            existing_user_by_email = await user_repo.get_by_email(email)
            if existing_user_by_email:
                raise ValueError("EMAIL_ALREADY_EXISTS")
            
            # Check if user already exists by phone number
            if phone:  # Only check if phone is provided
                existing_user_by_phone = await user_repo.get_by_phone(phone)
            ...
            # Save user with database constraint error handling
            try:
                created_user = await user_repo.create(user)
            except Exception as db_error:
                # Handle database constraint violations (race conditions, etc.)
